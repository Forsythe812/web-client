...
int crawled(int depth, const char *url) {
...
int get_url(char *url, SSL_CTX *ctx, int depth, int count, char **final_url, char **url_type) {
    if (count > 10) {
    ...
    if (result == ERR_FETCH_URL) {
        ...
        if (location) {
            ...
            if (end) {
                ...
                get_url(redirect_url, ctx, depth, count + 1, final_url, url_type);
                free(redirect_url);
                free(new_location);
            }
        }
...
int get_and_parse(char *url, int depth, SSL_CTX *ctx) {
    ...
    if (strcmp(url_type, "html") == 0 && depth < MAX_DEPTH) {
        ...
        snprintf(filepath, sizeof(filepath), "%s/%s.html", output_dir, read_filename);
        free(read_filename);
...
void child_process (SSL_CTX *ctx, int id) {
    while (1) {
        sem_wait(sem);
...
int get_url_content(char *url, char *dir) {
    ...
    crawled_data->depth[crawled_data->crawled_count] = NOT_CRAWLED;
    crawled_data->crawled_count++;
    //sem_post(sem);
